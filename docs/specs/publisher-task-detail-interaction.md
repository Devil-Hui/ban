...
| published | **手动调整** `goAdjust`（开发中 Toast） | 归档、分享预览 |
| adjusting | **继续调整** `goAdjust` | 回滚上版（开发中） |
...
| 开发中能力 | goAdjust / rollback / reopenSubmit → Toast「开发中」 |
...
#### btn_adjust · 手动调整 / 继续调整

- **可见**：published / adjusting  
- **触发**：Toast「调整页开发中，可先重新生成」  
- **A**：仅 Toast  
- **规格目标态**（未实现）：navigate scheme 调整或 adjust API  
- TC：`TC-task-detail-adjust-stub`  
...
| TC-task-detail-adjust-stub | published | 点手动调整 | Toast 开发中 |
